...
def get_nonlinearity(config: Optional[Union[str, dict]], **param):
    ...
    if isinstance(config, dict):
        ...
        param = {k: v for k, v in config.items() if k != 'type'}
        if name == 'siren':
...
class BatchDenseLayer(nn.Module):
    ...
    def __init__(self, weight: torch.Tensor, bias: torch.Tensor, activation: Union[str, dict] = None):
        ...
        if bias is not None:
            assert [*weight.shape[:-2]] == [*bias.shape[
                                             :-1]], f'weight and bias should have the same prefix shape, current: {[*weight.shape[:-2]]}, {[*bias.shape[:-1]]}'
            assert bias.shape[-1] == weight.shape[
                -2], f'bias should have shape suffix = {weight.shape[-2]}, but current is {bias.shape[-1]}'
        self.weight = weight
        self.bias = bias
